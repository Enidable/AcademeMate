...
// never gets re-added from the log on the next load.
  ...
  function updateGradeComponents(course, components) {
    ...
    let content = existingContent.map(i => {
      ...
      if (comp.dueDate) {
        ...
        next.date = null
        const label = comp.notes ? `${comp.id || comp.name} · ${comp.notes}` : (comp.id || comp.name)
    ...
    for (const c of entry.components) {
      ...
      content.push({
        ...
        topic: c.notes ? `${c.id || c.name} · ${c.notes}` : (c.id || c.name),
        description: c.notes ? `${c.id || c.name} · ${c.notes}` : (c.id || c.name),
        done: c.done || '',
        date: null,
        deadline: c.dueDate,
        start: '',
  ...
  function addAdditionalEntry(entry) {
    ...
    const row = {
      ...
      efficiency: entry.efficiency != null ? entry.efficiency : null,
      wellbeing: entry.wellbeing != null ? entry.wellbeing : null,
